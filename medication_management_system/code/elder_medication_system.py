...
def setup_medication_database():
    ...
    conn = sqlite3.connect(':memory:')  # Use file path for persistence
    cursor = conn.cursor()
    ...
    # Insert sample data
    sample_elders = [
        (1, "John Smith", 78, "555-0101", "Alice Smith (daughter)", "123 Main St"),
        (2, "Mary Johnson", 82, "555-0102", "Bob Johnson (son)", "456 Oak Ave"),
        (3, "Robert Brown", 75, "555-0103", "Carol Brown (wife)", "789 Pine Rd"),
    ]
    
    cursor.executemany(
        'INSERT INTO elders (elder_id, name, age, phone, emergency_contact, address) VALUES (?, ?, ?, ?, ?, ?)',
        sample_elders
    )
